...
/**
 * Fill in the Blank (`/vocabulary/blank/:shelf`) — one word per session,
 * `sessions = clamp(pool, 5, 10)`, on either shelf. A wrong answer never
 * shows the solution outright: the learner retries as often as they like,
 * with a separate « Reveal answer » escape hatch. A correct session
 * auto-advances after a short frozen pause.
 */
export default function BlankSessionPage() {
  const { user } = useAuth();
  const { shelf: shelfParam } = useParams();
  const shelf = parseShelf(shelfParam);
  const title = shelf === 'learned' ? 'Fill in the Blank · Review' : 'Fill in the Blank';
  ...
  const themeVars = vocabThemeVars('blank');
  ...
  function next() {
    if (isLast) {
      recordRound('vocabulary', roundMode('blank', shelf!), score, total);
      setFinished(true);
    } else {
      setIndex((i) => i + 1);
      setValue('');
